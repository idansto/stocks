...
def populate_general_metric_data_from_json(resourceName, global_metric_id):
    ...
    with open('../resources/'+resourceName, 'r') as file:
        ...
        first_new_line = read_thru_date(lines, latest_date)
        insert_row_to_global_data(first_new_line, global_metric_id)
        for line in lines:
            splitted_line = date_value_line(line)
            if (splitted_line):
                insert_row_to_global_data(splitted_line, global_metric_id)
    # connection.commit
    return


def insert_row_to_global_data(splitted_line, global_metric_id):
    (date_str, value) = splitted_line
