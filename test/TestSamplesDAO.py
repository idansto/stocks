...
class TestSamplesDAO(unittest.TestCase):
    ...
    def test_create_features_list(self):
        ...
        self.assertEqual(features, "(select d.value from shares.feature_data d where d.company_id = c.id and d.date = "
                                   "dates.date and d.feature_id = 1) as feature1,\n"+
                                   "(select d.value from shares.feature_data d where d.company_id = c.id and d.date = "
                                   "dates.date and d.feature_id = 2) as feature2,\n"+
                                   "(select d.value from shares.feature_data d where d.company_id = c.id and d.date = "
    ...
    def test_getSamplesSql(self):
        sql = get_samples_sql([1, 2, 3], ["2020-3-31", "2020-6-30"], [4, 5])
        print(sql)
